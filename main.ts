...
interface StatsData {
  dailyUsers: Array<{ timestamp: string; hwid?: string; ip?: string }>;
  keyVerifications: Array<{ timestamp: string; key?: string; result?: string }>;
  apiCalls: Map<string, number>;
  errors: Array<{ timestamp: string; error?: string; endpoint?: string }>;
}
...
// Система ролей
const USER_ROLES: { [key: string]: number } = {
  'user': 1,
  'premium': 2,
  'admin': 3
};
...
// Utility functions
// ----------------------
function getHwidIdentifier(req: Request): string {
  const hwid = req.body ? (req.body as any).hwid : null;
  return hwid || req.ip || 'unknown';
}
...
function getUserId(ip: string, hwid: string): string {
  return Buffer.from(`${ip}_${hwid}`).toString('base64');
}

function safeHtml(s: any): string {
  if (!s) return '';
  return s.toString()
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
...
function generateKey(length: number = 35): string {
...
async function triggerWebhooks(eventType: string, data: any): Promise<void> {
  const promises = WEBHOOK_URLS
    .filter(url => url.trim())
    .map(url =>
      axios.post(url, { event: eventType, data } as WebhookData, { timeout: 5000 })
        .catch((error) => {
...
function updateStats(eventType: string, data: any = null): void {
...
async function cleanupOldKeysAndUsers(): Promise<void> {
  while (true) {
    try {
...
async function scheduledTasks(): Promise<void> {
  while (true) {
    try {
      const now = new Date();
...
async function saveKey(key: string | null = null): Promise<string | null> {
...
// ----------------------
function requireAdmin(req: ExtendedRequest, res: Response, next: NextFunction): void {
...
app.get('/api/active_users', requireAdmin, (req: Request, res: Response) => {
...
app.post('/api/clean_old_keys', requireAdmin, [
  ...
  const days = parseInt((req.body as any).days) || 1;  
...
app.get('/api/verify_key', keyValidation, async (req: Request, res: Response) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(400).json({ errors: errors.array() });
  }
  ...
  try {  
    ...
    const created_at = new Date((data as Key).created_at);  
    const now = new Date();  
      
    if (now.getTime() - created_at.getTime() > 24 * 60 * 60 * 1000) {  
...
app.post('/api/save_user', userValidation, async (req: Request, res: Response) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(400).json({ errors: errors.array() });
  }
...
// Admin Panel Routes
// ----------------------
app.get('/user/admin', requireAdmin, (req: Request, res: Response) => {
...
app.post('/user/admin', [
  ...
  const password = (req.body as any).passwrd;
...
app.get('/api/admin/data', requireAdmin, async (req: Request, res: Response) => {
  try {
...
// ----------------------
app.get("/api/checkUpdate/KeySystem", (req: Request, res: Response) => {
  res.json({"update_available": false});
});

app.get("/api/AntiKick", (req: Request, res: Response) => {
  res.send("pastefy.app/0vPA1qOu/raw");
});

app.get("/api/GetScript/KeySystem", (req: Request, res: Response) => {
  res.json({'loadURL': ''});
});
...
app.post('/api/delete_user', requireAdmin, [
  ...
  const hwid = (req.body as any).hwid;  
...
// Выход из админки
app.post('/user/admin/logout', requireAdmin, (req: ExtendedRequest, res: Response) => {
